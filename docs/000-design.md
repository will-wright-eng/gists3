...
| `DeleteObject` semantics | *(added 2026-07-05 from live-API findings)* GitHub returns the same opaque 422 for three distinct "no effective change" cases. `DeleteObject` disambiguates with one GET: a missing key returns **success** (S3-idempotent); the sporadic rejection of deleting a duplicate-content file is absorbed by rewriting the file with unique content and re-deleting (one extra revision); deleting a gist's last file remains a clearly-worded `APIError`. |
...
Discovered during implementation (2026-07-05, live-API probing):

4. **GitHub 422s every "no effective change" PATCH identically** (`Validation Failed` / `missing_field: files`). Three triggers were isolated empirically: (a) null-deleting a key that doesn't exist; (b) *sporadically*, null-deleting a file whose content duplicates another file's, even though the file provably exists — the failure is sticky for that gist until the delete is bundled with another change or the content is first made unique; (c) deleting the last remaining file. Because `CopyObject` + `DeleteObject` composes exactly the duplicate-content pattern, `DeleteObject` now disambiguates a 422 with one GET and handles each case per the §5.4 row above. Same-content `PutObject` rewrites do **not** trip this (verified live), so no other method needs the workaround.

5. **The gist backend is eventually consistent.** Observed live under repeated runs: a GET immediately after a successful PATCH can miss the just-written file, visibility can regress between consecutive reads, and rapid sequential PATCHes to one gist sporadically return `409 Gist cannot be updated`. The no-retry stance (§5.3) stands — these surface honestly as `NotFoundError`/`APIError`. The integration tests absorb the lag with a bounded `eventually` poll; `WithRetry` (§9, v1.1) remains the roadmap answer for callers who want it handled.
